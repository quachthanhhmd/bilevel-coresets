...
usage() {
    ...
    echo "                              'coreset' xây coreset qua NTK proxy (cần jax/neural-tangents)."
    echo "                              'coreset_direct' xây coreset trực tiếp trên model đang huấn luyện"
    echo "                              (bicoreset.direct, không cần jax, nhưng chậm hơn nhiều vì phải huấn"
    echo "                              luyện lại model ở mỗi bước forward selection)."
    echo "                              'coreset_nystrom' giống 'coreset' nhưng dùng đúng kiến trúc Sec 5.1:"
    echo "                              CNTK 6 lớp + global average pooling (bicoreset/cntk.py) chiếu xuống"
    echo "                              không gian đặc trưng Nystrom (bicoreset/nystrom.py), rồi chọn coreset"
    echo "                              qua logistic regression proxy (cần jax/neural-tangents)."
